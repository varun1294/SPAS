<?php
	...
	/* For Marks Analysis */
	/* ************************************************************************************************************************************ */
		$sql3 = "SELECT COUNT(DISTINCT usn) FROM activity";
	
		$mydata3 = mysql_query($sql3,$con);
		$res = mysql_fetch_array($mydata3);
		...
		$sql3 = "SELECT * FROM marks WHERE courseid = '$courseId'";
		$mydata3 = mysql_query($sql3,$con);
		...
		$i = 0;
		$j = 0;
		while($res = mysql_fetch_array($mydata3)) {
			if($res['type'] == 'ia1')
